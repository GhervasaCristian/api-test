# 🌍 MeasurementUserManager - Monorepo (Next.js 15 + Turborepo + pnpm)
...
Proiectul simulează un sistem de gestionare a utilizatorilor pentru o platformă de **măsurători de mediu** (temperatură și umiditate).

---

## 🚀 Ghid de Instalare și Setup (pnpm)

Pentru a rula acest proiect, urmează pașii de mai jos într-un terminal deschis în rădăcina proiectului (`/MeasurementUserManager`):
...
## 🏗️ Arhitectura Proiectului (Monorepo)
...
*   `apps/UserCreator/`: Panoul de control administrativ (Next.js 15). Deține API-ul central și baza de date in-memory.
*   `apps/Client/`: Aplicația de vizualizare pentru utilizatorii finali (Next.js 15). Comunică cu `UserCreator` prin API.
...
O interfață curată cu o tematică vizuală de "mediu" (nuanțe de verde).
...
> **Comentariu pentru examen:** Proiectul demonstrează separarea responsabilităților (Separation of Concerns). `UserCreator` este "Producer-ul" de date, iar `Client` este "Consumer-ul", comunicând prin standardul REST API.
